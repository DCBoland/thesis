...
\begin{algorithm}[H]
 \KwData{\\
Voices - An array containing streams of note onset times for each voice in a piece of polyphonic music. \\
$A_{ff}$ - A sorted array of voice indices, indicating the user's ranked affinity for entraining with each voice.\\}\vspace{0.1in}
