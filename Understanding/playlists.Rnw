...
As the existing music retrieval systems used by listeners are based upon these features, it is likely that a `chicken-and-egg' effect will apply, where the features which best describe user playlists are those which users are currently exposed to in their existing retrieval interfaces. This approach requires discretised features, and is sensitive to discretisation strategy. The examples shown use features that were already discretised by the commercial provider.
